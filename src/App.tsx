...
function App({ list, sortFn, width = 200, height = 200 ,autoPlay=false}: { list: Array<number>, width?: number, height?: number, sortFn?: SortFn, ref?: any ,autoPlay?:boolean}) {
  ...
  const [canvasController, setCanvasController] = useState(new CanvasController());
  ...
  function draw() {
    ...
    sList.getList().forEach((n, i) => {
      ...
      context.lineWidth = (fieldWidth / (sList.getSize() * 2)) * canvasController.zoom;
    ...
    while (isSwap) {
      ...
      for (let i = 0; i < last; i++) {
        ...
        if (list.getList()[i] > list.getList()[i + 1]) {
          ...
          // await Main.Show(100);
          [list.getList()[i], list.getList()[i + 1]] = [list.getList()[i + 1], list.getList()[i]]
          await show(100);
          isSwap = true;
        } else {
          await show(100);
